...
public class LetterSoupTest {
	...
	for (int i = 0; i < testVectors.size(); ++i) {
	    ...
	    if (!ByteOperations.equals(result, cipher))
		error(vector.get("set"), vector.get("vector"));
	    if (!ByteOperations.equals(tag04, tag04result))
		error(vector.get("set"), vector.get("vector"));
	    if (!ByteOperations.equals(tag08, tag08result))
		error(vector.get("set"), vector.get("vector"));
	    if (!ByteOperations.equals(tag12, tag12result))
		error(vector.get("set"), vector.get("vector"));
	    if (!ByteOperations.equals(msg, decrypted))
		error(vector.get("set"), vector.get("vector"));
	}
